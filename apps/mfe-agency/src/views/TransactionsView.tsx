// FILE: apps/mfe-agency/src/views/TransactionsView.tsx
...
export const TransactionsView = ({ userData }: { userData: any }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard label="Transactions Agence" value={transactions.length} icon={<Banknote />} />
        <StatCard label="Revenus Locations" value={`${totalIncome.toLocaleString()} XAF`} icon={<ArrowUpRight className="text-green-500"/>} />
        <StatCard label="Remboursements" value={`${totalRefund.toLocaleString()} XAF`} icon={<ArrowDownRight className="text-red-500" />} />
      </div>
              ...
              return (
                <div key={tx.id} onClick={() => setSelectedTx(tx.id)} className="p-6 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-900/50 cursor-pointer transition-colors">
                  ...
                  <div className="text-right shrink-0 ml-4">
                    ...
                    <p className={`text-[8px] font-black uppercase tracking-widest mt-1 ${tx.status === 'COMPLETED' || tx.status === 'SUCCESS' ? 'text-green-600' : 'text-red-600'}`}>{tx.status}</p>
                  </div>
                </div>
              );
            })}
