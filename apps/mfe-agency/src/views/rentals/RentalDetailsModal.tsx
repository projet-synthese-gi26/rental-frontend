          ...
          <div className="p-6 md:p-10 overflow-y-auto custom-scrollbar space-y-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <section className="p-7 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 space-y-4">
                    <h4 className="text-[11px] font-black uppercase text-[#0528d6] italic border-b dark:border-slate-800 pb-2 flex items-center gap-2"><User size={14}/> {t.table.customer || 'Client'}</h4>
                    <DataRow label={t.auth.lastname || 'Nom'} value={rental.clientName} />
                    <DataRow label={t.agencies.card.phone || 'Contact'} value={rental.clientPhone} icon={<Phone size={10}/>} />
                    <DataRow label="ID / CNI" value={rental.cniNumber || 'N/A'} mono />
                </section>

                <section className="p-7 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 space-y-4">
                    <h4 className="text-[11px] font-black uppercase text-[#0528d6] italic border-b dark:border-slate-800 pb-2 flex items-center gap-2"><Clock size={14}/> Planning</h4>
                    <DataRow label={t.table.start || 'Prise en charge'} value={new Date(rental.startDate).toLocaleString()} />
                    <DataRow label={t.table.end || 'Restitution'} value={new Date(rental.endDate).toLocaleString()} />
                ...
                <section className="p-7 bg-[#0528d6] rounded-[2.5rem] text-white shadow-xl space-y-4 relative overflow-hidden">
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-center"><span className="text-[10px] font-bold opacity-60 uppercase">{t.table.total || 'Montant Total'}</span><span className="text-xl font-black">{rental.totalAmount?.toLocaleString()} XAF</span></div>
                        <div className="flex justify-between items-center"><span className="text-[10px] font-bold opacity-60 uppercase">{t.kpi.revenue || 'Déjà Encaissé'}</span><span className="text-xl font-black text-green-400">{rental.amountPaid?.toLocaleString()} XAF</span></div>
                        <div className="flex justify-between items-center pt-2 border-t border-white/10"><span className="text-[10px] font-bold opacity-60 uppercase">{t.table.remaining || 'Reste dû'}</span><span className="text-xl font-black text-orange-400">{(rental.totalAmount - rental.amountPaid).toLocaleString()} XAF</span></div>
                    </div>
                </section>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center gap-8 group text-center sm:text-left">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-[#0528d6] uppercase tracking-widest italic mb-1">{t.table.vehicle || 'Véhicule assigné'}</p>
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center gap-8 text-center sm:text-left">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-[#0528d6] uppercase tracking-widest italic mb-1">{t.sidebar.staff || 'Conducteur'}</p>
                        {driver ? (
                            <>
                        ...
                        ) : <p className="text-sm font-black text-slate-500 uppercase italic">{t.staff.noPoste || 'Aucun chauffeur assigné'}</p>}
                    </div>
                </div>
            </div>
            ...
            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                    ...
                    <div className="text-center md:text-left">
                        <p className="text-sm font-black uppercase italic tracking-widest leading-none">{t.system.log || 'Journal système'}</p>
