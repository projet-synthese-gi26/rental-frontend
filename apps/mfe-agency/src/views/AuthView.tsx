...
export const AuthView = ({ onAuth, lang, setLang, darkMode, toggleTheme }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!success) {
      setError("Identifiants incorrects ou accès agence non configuré.");
      setLoading(false);
    }
  };

  return (
    <div className="min-h-screen bg-[#f4f7fe] dark:bg-[#080b14] flex flex-col justify-center items-center p-4 md:p-10 transition-colors duration-500 relative">
      ...
      <div className="w-full max-w-[1000px] h-full max-h-[650px] min-h-[550px] grid lg:grid-cols-2 bg-white dark:bg-[#1a1d2d] rounded-[2.5rem] shadow-2xl border border-white dark:border-slate-800 overflow-hidden relative z-10 animate-in fade-in zoom-in duration-500">
        ...
        <div className="hidden lg:flex flex-col justify-between p-16 bg-[#0528d6] relative overflow-hidden text-white h-full">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-4xl font-bold tracking-tighter leading-tight mt-12">
               Portail Personnel <br />
               <span className="text-blue-300">Agence.</span>
            </h2>
            <p className="mt-6 text-blue-100/70 font-medium text-sm leading-relaxed">
              Gérez les départs, les retours et le stock de votre agence en temps réel.
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 md:p-16">
            <div className="max-w-md mx-auto h-full flex flex-col justify-center">
              ...
              <div className="mb-10 text-center lg:text-left shrink-0">
                <h3 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Connexion</h3>
                <p className="text-slate-400 text-sm mt-2 font-medium italic">Accédez à votre console d&apos;agence</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-6">
                <AuthInput label="Identifiant" icon={<Mail/>} type="email" placeholder="agent@rental.com" value={form.email} onChange={v => setForm({...form, email: v})} />
                ...
                <button 
                  disabled={loading}
                  className="w-full py-4 bg-[#0528d6] text-white rounded-xl font-bold text-sm shadow-xl shadow-blue-600/20 hover:bg-blue-700 transition-all flex items-center justify-center gap-3 mt-4"
                >
              ...
              <div className="mt-12 flex items-center gap-6 pt-6 border-t border-slate-50 dark:border-slate-800 w-full justify-center shrink-0">
                <button onClick={() => setLang(lang === 'FR' ? 'EN' : 'FR')} className="flex items-center gap-2 text-[10px] font-black text-slate-300 hover:text-slate-600 uppercase transition-colors italic">
