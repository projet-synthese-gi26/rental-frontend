...
import { X, Loader2, Hash, Layers, Binary } from 'lucide-react';
  ...
  <div className="space-y-1.5">
    ...
    <div className="relative group">
      ...
      <input type={type} required={required} value={value} placeholder={placeholder} onChange={e => onChange(e.target.value)} 
             className={`w-full ${icon ? 'pl-10' : 'px-4'} p-3 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-xl font-bold text-sm outline-none focus:border-[#0528d6] dark:text-white transition-all shadow-sm`} />
    </div>
  </div>
);