...
export const VehicleFormModal = ({ editingVehicle, categories, initialData, onSubmit, onClose, modalLoading, error }: any) => {
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }} 
          ...
          <div className="px-10 py-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-[#1a1d2d]">
            <div>
              ...
              <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest mt-1 italic">Assignation immédiate à votre agence</p>
            </div>
          ...
          <div className="p-10 overflow-y-auto custom-scrollbar space-y-12 text-left">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase italic ml-1 flex items-center gap-2"><Layers size={12}/> Catégorie</label>
          ...
          <div className="px-10 py-7 border-t border-slate-100 dark:border-slate-800 bg-slate-50/30 flex gap-4">
            ...
            <button disabled={modalLoading || uploading}
              ...
              {modalLoading ? <Loader2 className="animate-spin size-4" /> : "Confirmer l'inventaire"}
            </button>
          </div>
        </form>
      </div>
  ...
  <div className="space-y-1.5">
    ...
    <div className="relative group">
      ...
      <input type={type} required={required} value={value} placeholder={placeholder} onChange={e => onChange(e.target.value)} 
             className={`w-full ${icon ? 'pl-10' : 'px-4'} p-3 ${dark ? 'bg-white/10 border-white/10 text-white' : 'bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white'} rounded-xl font-bold text-sm outline-none focus:border-[#0528d6] transition-all shadow-sm`} />
    </div>
  </div>
);