      ...
      <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 text-left">
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSubmit(vehicle.id, formData); }} 
          ...
          <div className="px-10 py-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-[#0528d6] text-white">
            <div>
              <h3 className="text-xl font-black uppercase italic tracking-tighter">Switch Opérationnel</h3>
              <p className="text-[10px] opacity-70 font-bold uppercase italic">{vehicle.brand} {vehicle.model}</p>
            </div>
          ...
          <div className="p-10 space-y-8">
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase italic tracking-widest flex items-center gap-2"><AlertTriangle size={14}/> État de disponibilité immédiat</label>
              <div className="grid grid-cols-2 gap-3">
                {['AVAILABLE', 'MAINTENANCE'].map(s => (
                  <button key={s} type="button" onClick={() => setFormData({...formData, globalStatus: s})}
                          className={`p-4 rounded-2xl border-2 font-black text-xs uppercase italic transition-all ${formData.globalStatus === s ? 'bg-[#0528d6] border-[#0528d6] text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 space-y-4">
              <label className="text-[10px] font-black text-[#0528d6] uppercase italic tracking-widest flex items-center gap-2"><CalendarRange size={16}/> Planifier une période</label>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <span className="text-[8px] font-black text-slate-400 uppercase ml-1 italic">Date de début</span>
                ...
                <div className="space-y-1">
                  <span className="text-[8px] font-black text-slate-400 uppercase ml-1 italic">Date de fin</span>
            ...
            <div className="grid grid-cols-2 gap-6">
                <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase italic flex items-center gap-2"><Clock size={14}/> Prix / Heure</label>
                ...
                <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase italic flex items-center gap-2"><Banknote size={14}/> Prix / Jour</label>
