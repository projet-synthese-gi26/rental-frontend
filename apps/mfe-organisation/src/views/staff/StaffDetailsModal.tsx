          ...
          <div className="px-10 py-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50">
            <div className="flex items-center gap-5">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2 italic">ID Collaborateur: {data.id?.substring(0,12)}</p>
                </div>
            </div>
          ...
          <div className="p-10 overflow-y-auto custom-scrollbar space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <section className="space-y-6">
                    <h4 className="text-xs font-black uppercase text-[#0528d6] italic border-b pb-2 flex items-center gap-2"><Fingerprint size={16}/> Informations Contrat</h4>
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-black text-slate-800 dark:text-white uppercase italic">{data.status}</p>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section className="space-y-6">
                    <h4 className="text-xs font-black uppercase text-[#0528d6] italic border-b pb-2 flex items-center gap-2"><Lock size={16}/> Habilitations Poste</h4>
                    <div className="p-6 bg-[#0528d6] rounded-[2.5rem] text-white shadow-xl relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <p className="text-[10px] font-bold uppercase opacity-60 mb-1">Rôle Système</p>
                            <h5 className="text-xl font-black italic tracking-tighter uppercase mb-4">{data.poste?.name || 'Aucun poste'}</h5>
                            <div className="flex flex-wrap gap-2">
                                {data.poste?.permissions?.map((p: any) => (
                                    <span key={p.id} className="px-2 py-1 bg-white/10 rounded-lg text-[8px] font-black uppercase border border-white/10">{p.name}</span>
                                ))}
                            </div>
                        </div>
                    </div>
