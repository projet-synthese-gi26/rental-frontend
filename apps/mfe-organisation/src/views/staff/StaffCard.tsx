...
import { Mail, Store, Shield, Edit3, Trash2, Info } from 'lucide-react';
...
export const StaffCard = ({ staff, agencies, onEdit, onDelete, onView }: any) => {
  ...
  return (
    <div className="bg-white dark:bg-[#1a1d2d] rounded-[2rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all group flex flex-col h-full text-left">
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center gap-4">
          <div className="size-14 rounded-2xl bg-[#0528d6] text-white flex items-center justify-center font-black text-xl italic shadow-lg shadow-blue-600/20 shrink-0">
      ...
      <div className="space-y-3 mb-8 pt-4 border-t border-slate-50 dark:border-slate-800">
        <div className="flex items-center gap-3 text-[11px] font-bold text-slate-600 dark:text-slate-400 italic">
          <Mail size={14} className="text-slate-300" /> <span className="truncate">{staff.email}</span>
        </div>
        <div className="flex items-center gap-3 text-[11px] font-bold text-slate-600 dark:text-slate-400 italic">
          <Store size={14} className="text-slate-300" /> <span className="truncate">{agencyName}</span>
        </div>
      </div>
      ...
      <div className="flex gap-2 mt-auto pt-4 border-t border-slate-50 dark:border-slate-800">
        ...
        <button onClick={() => onDelete(staff.id)} className="p-2.5 text-slate-400 hover:text-red-500 bg-slate-50 dark:bg-slate-800 rounded-xl transition-all shadow-sm"><Trash2 size={16}/></button>
      </div>
    </div>
  );
};