...
import { 
  LayoutDashboard, Store, BarChart3, Users, Settings, LogOut, Search, Bell, 
  ChevronDown, Zap, ShieldCheck, Database, Plus, Download, MoreHorizontal, 
  Menu, X, Sun, Moon, Languages, Home, Lock, Loader2, Globe
} from 'lucide-react';
      ...
      <div className="flex-1 space-y-10">
        <div>
          ...
          <nav className="space-y-1 text-[10px] font-black uppercase tracking-[0.2em]">
            {[{ id: 'd', i: <LayoutDashboard size={18}/>, l: t.sidebar.dash, a: true }, { id: 's', i: <BarChart3 size={18}/>, l: t.sidebar.stats }].map(i => (
      ...
      <div className="min-h-screen bg-[#F8F9FC] dark:bg-[#0f1323] flex items-center justify-center p-6 transition-colors duration-300">
        <div className="w-full max-w-xl bg-white dark:bg-[#1a1d2d] rounded-[3.5rem] p-10 md:p-16 shadow-2xl border border-slate-50 dark:border-slate-800 relative">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl font-[900] italic uppercase tracking-tighter mb-4 leading-none">{t.auth.title}</h1>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-24 px-6 md:px-10 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl md:text-3xl font-[900] italic uppercase tracking-tighter leading-none italic">{t.header.title}</h2>
          </div>
          <div className="flex items-center gap-4">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-10 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className={`bg-white dark:bg-[#1a1d2d] p-8 rounded-[2.5rem] border ${k.h ? 'border-orange-500/20' : 'border-slate-50 dark:border-slate-800'} flex flex-col justify-between h-44 group hover:shadow-xl transition-all`}>
                <div className="flex justify-between items-start"><div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl">{k.i}</div>{k.g && <span className="text-[10px] font-black text-green-600">{k.g}</span>}{k.b && <span className="bg-orange-500 text-white text-[8px] font-black px-3 py-1 rounded-full italic uppercase">{k.b}</span>}</div>
                <div><p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{k.l}</p><p className="text-3xl font-[900] italic uppercase">{k.v}</p></div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            {/* Chart Area */}
            <div className="lg:col-span-2 bg-white dark:bg-[#1a1d2d] p-8 md:p-10 rounded-[3rem] border border-slate-50 dark:border-slate-800">
               <div className="flex justify-between mb-10"><div><h3 className="text-2xl font-[900] italic uppercase tracking-tighter">{t.charts.title}</h3><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{t.charts.sub}</p></div><button className="px-6 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl text-[10px] font-black uppercase italic text-slate-400">{t.charts.period}</button></div>
            ...
            {/* System Status Area */}
            <div className="bg-[#0f1323] p-10 rounded-[3rem] text-white flex flex-col justify-between h-full relative overflow-hidden">
               <Database className="absolute top-0 right-0 p-8 opacity-10" size={150}/>
               <div className="relative z-10">
                  <div className="flex items-center justify-between mb-10"><h3 className="text-xl font-black uppercase italic tracking-widest">{t.system.title}</h3><div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" /></div>
                  <div className="space-y-8 text-[9px] font-black uppercase tracking-[0.3em]">
                     <div><div className="flex justify-between mb-3 text-slate-400"><span>{t.system.sync}</span><span>98% {t.system.op}</span></div><div className="h-1.5 bg-white/10 rounded-full"><div className="w-[98%] h-full bg-blue-600 rounded-full shadow-[0_0_10px_#0528d6]" /></div></div>
                     <div><div className="flex justify-between mb-3 text-slate-400"><span>{t.system.load}</span><span>65% {t.system.loadSub || "Moderate"}</span></div><div className="h-1.5 bg-white/10 rounded-full"><div className="w-[65%] h-full bg-orange-500 rounded-full shadow-[0_0_10px_#f97316]" /></div></div>
                  </div>
               </div>
          ...
          {/* Table */}
          <div className="bg-white dark:bg-[#1a1d2d] rounded-[3.5rem] border border-slate-100 dark:border-slate-800 overflow-hidden">
            <div className="p-10 flex justify-between items-center border-b border-slate-50 dark:border-slate-800">
               <div><h3 className="text-2xl font-[900] italic uppercase tracking-tighter">{t.table.title}</h3></div>
            ...
            <div className="overflow-x-auto">
               <div className="min-w-[800px] px-10 pb-10">
                  <table className="w-full text-left italic font-black uppercase">
                    <thead><tr className="text-[10px] text-slate-300 border-b border-slate-50 dark:border-slate-800"><th className="py-8">{t.table.colName}</th><th>{t.table.colLoc}</th><th>{t.table.colStatus}</th><th>{t.table.colRev}</th></tr></thead>
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800 text-sm">
                       {[{ n: "Downtown Hub", l: "New York, NY", s: "Online", v: "$45,200" }].map((row, i) => (
                          <tr key={i}><td className="py-8 text-slate-900 dark:text-white">{row.n}</td><td className="text-slate-400 text-xs tracking-widest">{row.l}</td><td><span className="flex items-center gap-2 tracking-widest text-[10px]"><div className="w-2 h-2 rounded-full bg-green-500" />{row.s}</span></td><td className="text-blue-600">{row.v}</td></tr>
                       ))}
                    </tbody>
                  </table>
               </div>
