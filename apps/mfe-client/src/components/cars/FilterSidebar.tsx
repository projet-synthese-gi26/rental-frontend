    ...
    <div className={isMobile ? mobileClasses : desktopClasses}>
      <div className="flex justify-between items-center border-b-2 border-slate-50 dark:border-slate-800 pb-4 shrink-0">
        <div className="flex items-center gap-2 text-[#0528d6]">
          <Filter size={18} />
          <h4 className="font-bold uppercase italic text-xs tracking-widest">Filtrage</h4>
        </div>
        <button onClick={onReset} className="text-[9px] font-black text-slate-400 uppercase hover:text-red-500 transition-colors">Reset</button>
      </div>

      {/* Zone scrollable interne */}
      <div className="flex-1 overflow-y-auto no-scrollbar py-8 space-y-10">
        <section className="space-y-4 text-left">
          <label className="text-[10px] font-bold uppercase text-slate-500 italic flex items-center gap-2">
      ...
      <div className="pt-6 border-t-2 border-slate-50 dark:border-slate-800 shrink-0">
           <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-100 dark:border-blue-800/50">
              <p className="text-[9px] font-black text-[#0528d6] leading-relaxed uppercase italic">
...
const CategoryBtn = ({ label, active, onClick }: any) => (
  <button onClick={onClick} className={`flex items-center justify-between px-5 py-3 rounded-xl text-[10px] font-black uppercase border-2 transition-all ${active ? 'bg-[#0528d6] border-[#0528d6] text-white' : 'bg-slate-50 dark:bg-slate-900 border-transparent text-slate-400 hover:bg-slate-100'}`}>
...
const SelectBtn = ({ label, active, onClick }: any) => (
  <button type="button" onClick={onClick} className={`py-3 rounded-xl text-[10px] font-black uppercase border-2 transition-all ${active ? 'border-[#0528d6] bg-[#0528d6] text-white shadow-md' : 'border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-400 hover:border-slate-300'}`}>
    {label}
  </button>
);